...
// Subcommand for checking multiple connections — `--date` is parent-only;
// subcommand reads it via optsWithGlobals() to avoid commander v12 shadowing.
...
async function handleFindCommand(
  ...
  const connections = checker.getFeasibleConnections(from, to, via, date, minConfidence);
...
// Subcommand for quick check — `--date` is parent-only; see findCommand.
...
async function handleQuickCommand(
  ...
  for (const station of commonStations) {
    const connection = checker.checkConnection(train1, train2, station, date);
